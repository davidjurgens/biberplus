...
from bibermda.tagger.tagger_parallel import tag_string_parallel

from collections import defaultdict
from bibermda.analyzer.constants import ALL_TAGS, DOC_TAGS
from bibermda.tagger.tagger import tag_string_batched

from math import ceil


def calculate_corpus_statistics(text, token_batch_size=10000, token_normalization=1000, use_gpu=True,
                                show_progress=False):
    ...
    for word_tagger in tag_string_batched(pipeline, text, token_batch_size, show_progress):
...
def calculate_corpus_statistics_parallel(text, token_batch_size, token_normalization=1000, n_processes=4, use_gpu=True,
                                         show_progress=False):
    corpus_tag_counts = defaultdict(list, {k: [] for k in ALL_TAGS})
    corpus_doc_counts = defaultdict(list, {k: [] for k in DOC_TAGS})

    for tagged_words in tag_string_parallel(text, token_batch_size, n_processes,
