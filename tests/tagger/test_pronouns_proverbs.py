...
class TestPronounProverbFunctions(unittest.TestCase):
    ...
    def test_spp2(self):
        text = (
            ". . -RRB- The new interpretation makes sense though if you think about it . "
            "By the way , he"
        )
        tagged_words = tag_text(text, pipeline=self.pipeline)
    ...
    def test_pit(self):
        text = (
            "sometimes answers itself , and that the way in which it is posed frequently shapes "
    ...
    def test_demp(self):
        text = (
            "Vernon on the morning of the regular tallyho run . This was an honor , "
    ...
    def test_fpp1_variants(self):
        ...
        for text, target_word in texts:
            tagged_words = tag_text(text, pipeline=self.pipeline)
            word_index = next(
                i for i, w in enumerate(tagged_words) if w["text"] == target_word
            )
    ...
    def test_prod_patterns(self):
        ...
        for text in texts_should_be_prod:
            ...
            prod_tokens = [
                ...
                if w["text"].lower() in ["do", "does", "did", "done"]
                and "PROD" in w["tags"]
            ]
            self.assertTrue(
                len(prod_tokens) > 0,
        ...
        for text in texts_should_not_be_prod:
            ...
            self.assertEqual(
                len(prod_tokens),
                0,
                f"Did not expect PROD tag in any 'do' token in: {text}",
            )
