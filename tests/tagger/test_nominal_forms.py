...
class TestNominalFormFunctions(unittest.TestCase):
    ...
    def test_nomz_basic(self):
        text = (
            "consular materials to reveal the motives which led the British government to permit Garibaldi to cross "
    ...
    def test_ger(self):
        text = (
            "His voice carries the album well even with some subpar songwriting . I do n't know where "
            "people are getting"
        )
        tagged_words = tag_text(text, pipeline=self.pipeline)
