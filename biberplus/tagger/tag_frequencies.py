...
def calculate_tag_frequencies(text, pipeline=None, config=None):
    ...
    try:
        ...
        tag_frequencies = count_tags_every_n_tokens(
            tagged_dataframe, tag_frequencies, tags, config
        )
...
def count_tags_every_n_tokens(tagged_df, tag_counts, tags, config):
    ...
    for index, batch in enumerate(np.array_split(tagged_df, num_batches)):
        ...
        if (
            last_batch
            and len(batch)
            <= config["drop_last_batch_pct"] * config["token_normalization"]
        ):
            break

        weight = config["token_normalization"] / len(batch) if last_batch else 1.0
        tag_counts = update_tag_counts(
            batch, tag_counts, tags, tag_binary=config["binary_tags"], weight=weight
        )

    return tag_counts


def update_tag_counts(tagged_df, tag_counts, tags, tag_binary, weight=1.0):
    curr_counts = (
        pd.Series(
            functools.reduce(operator.iconcat, tagged_df.tags, []),
...
def load_tags(config):
    ...
    if config["function_words"]:
        fw = (
            config["function_words_list"]
            if config["function_words_list"]
            else build_variable_dictionaries()["function_words"]
        )
        tags.extend(fw)
