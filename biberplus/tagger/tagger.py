...
def tag_text(text, pipeline=None, config=None):
    ...
    for text_batch in simple_split_batching(
        text, config["processing_size"], config["show_progress"]
    ):
...
def tag_text_parallel(text, config):
    ...
    for text_batch in simple_split_batching(
        text, config["processing_size"], show_progress=False
    ):
