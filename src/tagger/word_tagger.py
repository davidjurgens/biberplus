...
class WordTagger:
    ...
    def tag_pred(self, word, previous_words, next_words):
        ...
        if self.helper.is_adjective(word):
            ...
            if previous_words[1] and self.tag_pred(previous_words[1], previous_words[2:], new_next_words) \
        ...
        if txt in self.patterns['public_verbs'] or txt in self.patterns['private_verbs'] \
            ...
            if next_words[2]:
                if next_words[2]['xpos'] in ['JJ', 'DT', 'CD', 'PRP$'] or self.helper.is_adverb(next_words[2]) or \
                        self.tag_pred(next_words[0], previous_words.append(word),
                                      next_words[1:]) or self.helper.is_quantifier(next_words[0]):
