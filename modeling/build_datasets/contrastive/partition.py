...
def sample_text_pairs(df):
    ...
    samples_Y = (
        df[~df["documentID"].isin(samples_X["documentID"])]
        .groupby(["authorID"])
        .sample(1)
    )
    del df
    ...
    assert (
        set(samples_Y["documentID"].tolist()).intersection(
            set(samples_X["documentID"].tolist())
        )
        == set()
    )
    assert len(samples_X["authorID"].sort_values().tolist()) == len(
        samples_Y["authorID"].sort_values().tolist()
    )
...
def save_text_samples(samples_X, samples_Y, partition, input_file, output_path):
    ...
    for key, value in partition.items():
        samples_X_partition = set(
            samples_X[samples_X["authorID"].isin(partition[key])]["documentID"]
            .astype("str")
            .tolist()
        )
        samples_Y_partition = set(
            samples_Y[samples_Y["authorID"].isin(partition[key])]["documentID"]
            .astype("str")
            .tolist()
        )
