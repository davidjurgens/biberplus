...
def build_data_split(type, output_path):
    ...
    align(
        os.path.join(output_path, f"{type}_candidates.jsonl"),
        os.path.join(output_path, f"{type}_queries.jsonl"),
    )
    ...
    create_dataset(
        os.path.join(output_path, f"{type}.jsonl"), os.path.join(output_path, type)
    )
