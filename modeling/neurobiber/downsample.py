...
def sample_max_lines(datasets, output_dir, max_samples=1000000):
    ...
    for dataset in datasets:
        ...
        print(
            f"Completed: Read {lines_read:,} lines, wrote {lines_written:,} lines for {dataset}"
        )
    ...
    print(
        f"Total samples: {total_written}, Train: {train_count}, Dev: {dev_count}, Test: {test_count}"
    )
...
def main():
    ...
    total_samples, train_count, dev_count, test_count = sample_max_lines(
        datasets, output_dir, max_samples=1000000
    )
    print(
        f"Total samples: {total_samples}, Train: {train_count}, Dev: {dev_count}, Test: {test_count}"
    )
