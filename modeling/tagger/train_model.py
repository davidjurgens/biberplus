...
TAG_PERFORMANCE_SUMMARY_PATH = os.path.join(
    RUN_DIR, "tag_level_performance_summary.csv"
)
...
train_dataset = StreamingDataset(
    "/shared/3/projects/hiatus/tagged_data/binary_train.jsonl",
    tokenizer,
    max_length=512,
)
dev_dataset = StreamingDataset(
    "/shared/3/projects/hiatus/tagged_data/binary_dev.jsonl", tokenizer, max_length=512
)
test_dataset = StreamingDataset(
    "/shared/3/projects/hiatus/tagged_data/binary_test.jsonl", tokenizer, max_length=512
)

logger.info(f"Loading tokenizer and model from {MODEL_NAME}...")
model = AutoModelForSequenceClassification.from_pretrained(
    MODEL_NAME, problem_type="regression", num_labels=len(train_dataset.label_columns)
).to("cuda")

logger.info(
    f"Dataset sizes - Train: {train_dataset.total_rows}, Dev: {dev_dataset.total_rows}, Test: {test_dataset.total_rows}"
)
