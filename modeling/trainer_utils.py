...
from modeling.contrastive_training.collator import (
    ContrastiveBiberCollator,
    ContrastiveCollator,
)
...
def get_dataloaders(args, pairs=True, mlm=False, biber=False):
    ...
    collator, eval_collator = get_collators(
        tokenizer, args.max_length, mlm, pairs, biber
    )
...
def create_collators(CollatorClass, tokenizer, max_length):
    ...
    eval_collator = CollatorClass(
        tokenizer=tokenizer, max_length=max_length, evaluate=True
    )
...
def load_datasets(args):
    ...
    dev_dataset = load_dataset("json", data_files=args.dev_data, split="train").shuffle(
        seed=42
    )
